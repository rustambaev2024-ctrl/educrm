...
- [ ] **Step 7: Экран заявки — перебрендировать, но оставить тёмным**

**Поправка к первой версии плана.** Здесь было написано «два публичных
экрана: вход и заявка, оба намеренно тёмные». Это неверно, проверено
на живой странице при осмотре на 375px:

- **Вход (`/`) светлый** и уже работает на токенах (`bg-background`,
  `bg-gradient-hero`). Отдельного обращения не требует — переключится
  вместе с темой, как любой другой экран. Проверить нужно только
  `bg-gradient-hero`: он на старом синем бренде.
- **Тёмный целиком только `/apply`** — блок `.apply-page` и `.apply-*`,
  в самом CSS он так и подписан: `/* ── Apply / Landing page ── */`.
  Там же, кстати, уже стоит `100dvh`, а не `100vh`.

`/apply` остаётся намеренно тёмным в обеих темах: это лендинговая форма,
её видят до входа, тему пользователь ещё не выбирал. Токенизировать её
под переключение не нужно — нужно снять **старый синий бренд**, иначе
первый экран, который видит клиент, останется в прежних цветах.
...
```css
  /* ── Экран заявки (/apply) ──
     Намеренно тёмный в обеих темах: лендинговая форма, её видят до входа,
     тему пользователь ещё не выбирал. Поэтому у неё свои фиксированные
     значения бренда, не зависящие от --primary. Это не «забытый хардкод». */
  --public-bg: #070D0A;
  --public-brand: #2CC98D;
...
Дальше заменить по таблице внутри блока `.apply-*` (найти границы
по комментарию `/* ── Apply / Landing page ── */` — номера строк
сдвинутся после задач 4–6, поэтому ориентироваться на него, а не на числа):
...
Отдельно глазами, **на 375px** — проектный скил `design-system` требует
этого прямо, потому что оба мобильных бага в истории проекта прошли
статическое ревью и были найдены только по скриншотам из прода:

- `/apply` — остался тёмным и читаемым, но в зелёном бренде, не в синем
- `/` (вход) — переключается вместе с темой и не потерял `bg-gradient-hero`

У остальных экранов видимых изменений в этом плане нет по построению.
