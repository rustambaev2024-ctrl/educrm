...
`test_phase4_api.py::test_homework_grade_exam_cycle` (строка 96) отправляет `"grade": 9` при создании ДЗ и `"score": 9` для экзамена с `"max_score": 10` — эти значения (9) теперь ВНЕ диапазона 2-5 и будут отклонены новым валидатором (Task 2) или сериализатором HomeworkStatus (не имеет собственного range-валидатора на `grade`, но `Grade`, создаваемый через `upsert_homework_grade`, не проходит через `GradeSerializer.validate_score` — сервис пишет напрямую в модель, а не через сериализатор, так что серверная валидация здесь не сработает автоматически; тест, скорее всего, всё ещё пройдёт технически, но проверяет уже нерелевantное поведение).
