...
def daily_lesson_charge():
    """Плановое списание по всем тенантам (beat). Идемпотентна по (student, group, день)."""
    for schema in _iter_tenant_schemas():
        charge_tenant(schema)
...
def daily_lesson_charge_for_tenant(schema: str):
    """R-22: ручной запуск списания — строго в одной схеме, вызывающей запрос.

    Никогда не итерирует чужие схемы: деньги одного учебного центра не могут быть
    затронуты действием пользователя другого центра.
    """
    charge_tenant(schema)


def charge_tenant(schema: str):
    # localdate, а не date.today(): контейнер живёт в UTC, а `created_at__date`
    # сравнивается в TIME_ZONE проекта. Расхождение на границе суток ломало
    # гард идемпотентности и давало повторное списание (R-22).
    today = timezone.localdate()
    weekday = today.weekday()
    logger.info(f"daily_lesson_charge started for schema '{schema}', date {today}")
