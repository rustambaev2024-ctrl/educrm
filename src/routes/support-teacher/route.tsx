...
function SupportTeacherLayout() {
  ...
  const groups: RailGroup[] = [
    {
      ...
      sidebar: [{ id: "dashboard", to: "/support-teacher", icon: LayoutDashboard, label: tr("Bugun", "Сегодня") }],
    },
    {
      rail: { id: "teaching", icon: Layers, label: tr("Ta'lim", "Обучение") },
      sidebar: [
        { id: "groups", to: "/support-teacher/groups", icon: Layers, label: tr("Guruhlar", "Группы"), section: tr("TA'LIM", "ОБУЧЕНИЕ") },
        { id: "attendance", to: "/support-teacher/attendance", icon: ClipboardCheck, label: tr("Davomat", "Посещаемость"), section: tr("TA'LIM", "ОБУЧЕНИЕ") },
        { id: "homework", to: "/support-teacher/homework", icon: BookOpen, label: tr("Vazifalar", "Задания"), section: tr("TA'LIM", "ОБУЧЕНИЕ") },
        { id: "grades", to: "/support-teacher/grades", icon: Award, label: tr("Baholar", "Оценки"), section: tr("TA'LIM", "ОБУЧЕНИЕ") },
      ],
    },
    {
      rail: { id: "messages", icon: MessageSquare, label: tr("Xabarlar", "Сообщения") },
      sidebar: [
        { id: "messages", to: "/support-teacher/messages", icon: MessageSquare, label: tr("Xabarlar", "Сообщения"), section: tr("ALOQA", "СВЯЗЬ") },
      ],
    },
  ];
  ...
  for (const g of groups) {
    for (const leaf of g.sidebar) {
      const matches = path === leaf.to || (leaf.to !== "/support-teacher" && path.startsWith(`${leaf.to}/`));
      if ((matches || (leaf.to === "/support-teacher" && path === "/support-teacher")) && leaf.to.length > bestLen) {
